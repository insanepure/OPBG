<?php
    if(!$player->GetInventory()->HasGanfortKey())
    {
        echo 'Du hast kein Recht hier rein zu kommen! Dieses Recht musst du dir erst verdienen!';
    }
    else
    {
        ?>
        <img width='100%' height='300' src='img/marketing/Shop.png' />
          ...
          <div class="schatten">Suchen</div>
            ...
            <tr>
                <form method="GET" action="?p=shop">
                    ...
                    <td width="40%">
                        <input style="width:90%;" type="text" name="itemname" placeholder="Item" value="<?php if (isset($_GET['itemname'])) echo htmlentities($_GET['itemname']); ?>">
                    </td>
                    <td width="40%">
                ...
                usort($items, function ($a, $b) use ($itemManager)
                {
                    ...
                    if($item1->GetType() == $item2->GetType())
                        if($item1->GetPrice() == $item2->GetPrice())
                            if($item1->GetLP() == $item2->GetLP())
                                return $item1->GetKP() <=> $item2->GetKP();
                            else
                                return $item1->GetLP() <=> $item2->GetLP();
                        else
                    ...
                    <tr>
                        ...
                        <td class="borderT" align="center">
                            <?php
                                ...
                                if ($item->GetLevel() != 0)
                                    echo '<br/>Benötigt Level ' . number_format($item->GetLevel(),'0', '', '.');
                            ?>
                        </td>

                        <td class="borderT" align="center"><?php echo number_format($item->GetPrice(), 0, ',', '.'); ?> <img src="img/offtopic/BerrySymbol.png" alt="Berry" title="Berry" style="position: relative; top: 2px; height: 20px; width: 13px;"/></td>
                        <td class="borderT">
                            <button onclick="OpenPopupPage('Item Kaufen','shop/ganfortshop.php', 'item=<?php echo $item->GetID();
