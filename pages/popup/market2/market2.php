...
<?php
$wartung = false;
if($wartung == false || $player->GetArank() >= 2) {
    ?>
    <img width='100%' height='150' style='margin-top:-10px' src='img/marketing/Marktplatz.png' />
    <!-- SUCHEN -->
    <table width="98%">
        <tr>
            <td width="70%">
    ...
    <div id="sellContainer" style="clear: left; float: left; margin-left: 1%; width: 27%; height: 500px;">
        <form method="POST" style="height: 100%;" action="?p=profil&a=sell">
            <div class="spacer"></div>
            <div id="itemContainer" style="width: 100%; height: 75px; white-space: nowrap;">
                ...
                <div id="itemContainerImage" class="boxSchatten borderL borderT borderR borderB" style="width: 60px; height: 60px; float: left; position: relative; left: 5%;">
                    ...
                    {
                        if($item->IsEquipped() || !$item->IsMarktplatz())
                        {
                            ++$i;
                            $item = $inventory->GetItem($i);
                        }
                ...
                <div id="itemContainerName" class="boxSchatten borderL borderT borderR borderB" style="width: fit-content; min-width: 100px; max-width: 215px; overflow-x: auto; height: fit-content; margin-top: 5px; margin-left: 20px; padding: 15px 10px 15px 10px; position: relative;">
                    <?php
                    if(isset($item) && !$item->IsEquipped() && $item->IsMarktplatz())
                        echo $item->GetName();
                    ?>
                </div>
            </div>
    ...
    <table width="70%" cellspacing="0" border="0" style="margin-right: 1%;">
        ...
        <tr>
            <?php if(true)
            {
                ...
                ?>
                <td width="10%"  align="center"><b>Bild</b></td>
                <td width="10%" style="cursor:pointer;" align="center" onclick="sortTable('name')"><b>Name</b></td>
                <td width="24%"  align="center"><b>Wirkung</b></td>
                <td width="13%"  align="center"><b>Dauer</b></td>
                <td width="15%" style="cursor:pointer; overflow: visible;" align="center" onclick="sortTable('seller')"><b>Besitzer / Käufer</b></td>
                <td width="14%" style="cursor:pointer;" align="center" onclick="sortTable('price')"><b>Preis</b></td>
                <td width="14%"  align="center"><b>Aktion</b></td>
                <?php
            }
            ?>
        </tr>
    </table>
    <div id="MarketContainer" style="border:0px solid red; width:70%; height:480px; overflow-y:auto; overflow-x: hidden; margin-right: 1%;">
        <?php
        $i = 0;
        ...
        while (isset($marketItem))
        {
            ...
            <div class="MarketAnzeigenContainer">
                <div style="width:10%; height:100%; float:left;">
                    ...
                    {
                        ?>
                        <img class="boxSchatten borderT borderR borderL borderB" src="img/items/<?php echo $marketItem->GetOverlay(); ?>.png" alt="<?php echo $marketItem->GetName(); ?>" title="<?php echo $marketItem->GetName(); ?>" style="width:50px; height:50px; position:relative; z-index:1;">
                        <?php
                    }
                    ?>
                    <img class="boxSchatten borderT borderR borderL borderB" src="img/items/<?php echo $marketItem->GetImage(); ?>.png" alt="<?php echo $marketItem->GetName(); ?>" title="<?php echo $marketItem->GetName(); ?>" style="width:50px; height:50px; position:relative; z-index:0;">
                </div>
                ...
                <input type="hidden" class="name" value="<?php echo $marketItem->GetName(); ?>">
                ...
                <!--<div style="width:15%; float:left;">
                    <?php
                if($marketItem->GetKaeufer() != 0 && $marketItem->GetKaeufer() == $player->GetID() || $marketItem->GetSellerID() == $player->GetID())
                {
                    ?>
                                <a href="?p=profil&id=<?php echo $marketItem->GetSellerID(); ?>"><?php echo $marketItem->GetSeller(); ?></a>
                                <?php
                    $buyer = new Player($database, $marketItem->GetKaeufer());
                    ?>
                                <br/>
                                <a href="?p=profil&id=<?php echo $buyer->GetID(); ?>"><?php echo $buyer->GetName(); ?></a>
                                <?php
                } else {
                    echo "Anonym";
                }
                ...
                <div style="width:19%; float:left;">
                    <?php if($marketItem->GetSellerID() == $player->GetID())
                    {
                        ...
                        {
                            ?>
                            <button onclick="OpenPopupPage2('Item Nehmen','market2/retake.php?p=<?php echo $_GET['p']; ?>&item=<?php echo $marketItem->GetID(); ?>')">
                                Nehmen
                            </button>
                            <?php
                        }
                    ...
                    {
                        ?>
                        <button onclick="OpenPopupPage2('Item Kaufen','market2/buy.php?p=<?php echo $_GET['p']; ?>&item=<?php echo $marketItem->GetID(); ?>')">
                            Kaufen
                        </button>
                        <?php
                    }
                    if($player->GetArank() == 3)
                    {
                        ?>
                        <br/>
                        <button onclick="OpenPopupPage2('Item Entfernen','market2/remove.php?p=<?php echo $_GET['p']; ?>&item=<?php echo $marketItem->GetID(); ?>')">
                            Entfernen
                        </button>
                        <?php
