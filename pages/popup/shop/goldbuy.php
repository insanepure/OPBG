...
Preis: <b> <span id="price"><?= number_format($item->GetPrice(), '0', '', '.') ?></span> <img src="img/offtopic/GoldSymbol.png" alt="Gold" title="Gold" style="position: relative; top: 5px; height: 20px; width: 20px;"/></b>
<hr>
<div class="spacer"></div>
<?php
...
?>
<form method="POST" action="?p=goldshop&a=buy<?php if(isset($_GET['itemname'])) echo '&itemname='.$_GET['itemname']; if(isset($_GET['itemcategory'])) echo '&itemcategory='.$_GET['itemcategory']; ?>">
    ...
    Anzahl: <select style="width:100px" class="select" name="amount" onchange="setAmount(this,<?=$item->GetPrice()?>)">
        ...
        $j = 1;
        if(!in_array($item->GetID(), $shipArray)) $amount = 99;
        else $amount = $j;
        while($j <= $amount)
        {
            ?>
