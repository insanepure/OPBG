...
function maximize(fieldName, wert)
{
var totalFieldElement = document.getElementById(totalField);
totalStats = 0;
totalFieldElement.innerHTML = totalStats.toLocaleString();
lpFieldElement.value = 0;
kpFieldElement.value = 0;
aFieldElement.value = 0;
dFieldElement.value = 0;
var fieldElement = document.getElementById(fieldName);
fieldElement.value = wert;
valueMap.set('lp', Number(lpFieldElement.value));
valueMap.set('kp', Number(kpFieldElement.value));
valueMap.set('attack', Number(aFieldElement.value));
valueMap.set('defense', Number(dFieldElement.value));
...
function changeStats(fieldName, previousValue, newValue)
{
  ...
  var totalStats = Number(totalFieldElement.innerHTML.replace(".", ""));
