...
else
{
    ...
    if(!$player->IsAdminLogged()) {
        $isAdmin = 0;
        $result = $database->Select('*', 'accounts', 'userid='.$player->GetUserID().' AND arank >= 2');
        if($result && $result->num_rows > 0)
            $isAdmin = 2;
        $chat->AddUser($player->GetID(), $player->GetUserID(), $game, $player->GetName(), $isAdmin, $channel, session_id(), $titelText, $titelColor);
    }
    ?>
