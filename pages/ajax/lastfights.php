...
<div style="width: 600px; height: 632px; overflow-y: auto; float:left;">
    <table width="100%">
        <?php
            $fightslist = new GeneralList($database, 'lastfights', '*', 'fighters LIKE "%['.$_GET['id'].']%"', 'id', 99999, 'ASC');
            ...
            while($entry != null)
            {
                ...
                <tr>
                    <td class="borderB">
                        <a target="_blank" href="?p=lastfights&fight=<?php echo $entry['id']; ?>"><?php echo date('H:i:s', strtotime($entry['time'])); ?></a>
                    </td>
                    <td class="borderB">
                        <a target="_blank" title="log" href="?p=lastfightslog&fight=<?php echo $entry['id']; ?>"><?php echo $entry['id']; ?></a>
                    </td>
                    <td class="borderB">
                        <a target="_blank" href="?p=lastfights&fight=<?php echo $entry['id']; ?>"><?php echo Fight::GetTypeName($entry['type']); ?></a>
                    </td>
                    <td class="borderB">
                        <?php
                            if($entry['type'] != 1 && $entry['type'] != 13)
                                echo '<a target="_blank" href="?p=lastfights&fight=' . $entry['id'] . '">' . $entry['name'] . '</a>';
