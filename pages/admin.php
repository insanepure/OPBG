...
$limitedTables = array(
	'actions', 'attacks', 'items', 'npcs', 'places', 'story', 'sidestory', 'events', 'titel',
	'patterns', 'planet', 'changelog', 'passives', 'verzeichnis', 'towerfloors',
	'eventitems', 'skilltree'
);
...
if (isset($_GET['a']) && $_GET['a'] == 'delete' && isset($_POST['sure']))
{
	...
	$message = 'Die ID ' . number_format($id, '0', '', '.') . ' wurde aus Tabelle ' . $table . ' gelöscht.';
...
else if (isset($_GET['a']) && $_GET['a'] == 'edit')
{
	...
	if ($create && $table == 'changelog')
	{
        ...
        $chat->SendMessage('/system '.$_POST['test']);
	}
		...
		foreach ($finfo as $val)
		{
			...
			if ($row[$name] != $_POST[$name])
			{
				if(is_numeric($row[$name]))
					$wertalt = number_format($row[$name], 2, ',', '.');
				else
					$wertalt = $row[$name];

				if(is_numeric($_POST[$name]))
					$wertneu = number_format($_POST[$name], 2, ',', '.');
				else
					$wertneu = $_POST[$name];
				$log = $log . ' Setze Wert <b>' . $name . '</b> von <b>' . $wertalt . '</b> zu <b>' . $wertneu . '</b><br/>';
			}
		}
