...
{
  ?><a onclick="RemoveTableRow(this)" style="cursor: pointer;">X</a><?php
}
else if(isset($_GET['table']) && $_GET['table'] == 'playerinventory')
{
  ...
  if($_GET['cell'] == 0)
  {
		...
		{
		?>
			<option value="<?php echo $entry['id']; ?>"><?php echo $entry['name'].'('.number_format($entry['id'], '0', '', '.').')'; ?></option>
			<?php
			++$id;
			$entry = $items->GetEntry($id);
		}
...
else if(isset($_GET['table']) && ($_GET['table'] == 'attacks' || $_GET['table'] == 'removeattacks' || $_GET['table'] == 'addattacks' || $_GET['table'] == 'fightattacks' || $_GET['table'] == 'learnableattacks' || $_GET['table'] == 'needattacks' || $_GET['table'] == 'playerattack'))
{
  $row = $_GET['row'];
  if($_GET['cell'] == 0)
  {
		?>
		<select class="select" name="<?php echo $_GET['table']; ?>[<?php echo $i; ?>]" style="width:400px;">
...
else if(isset($_GET['table']) && ($_GET['table'] == 'needitems'))
{
  $row = $_GET['row'];
  if($_GET['cell'] == 0)
  {
		?>
		<select class="select" name="needitems[<?php echo $i; ?>]" style="width:400px;">
			<option value="0">Kein Item(0)</option>
		<?php
...
else if(isset($_GET['table']) && $_GET['table'] == 'spcialtrainuses')
{
    $row = $_GET['row'];
    if($_GET['cell'] == 0)
    {
        ?>
        <select class="select" name="spcialtrainuses[<?php echo $i; ?>]" style="width:400px;">
            <?php
                if($specialActions == null)
                {
                    $specialActions = new Generallist($database, 'actions', '*', 'type=15', '', 99999999999, 'ASC');
                }
                $id = 0;
                $entry = $specialActions->GetEntry($id);
                while($entry != null)
                {
                    ?>
                    <option value="<?php echo $entry['id']; ?>"> <?php echo $entry['name'].'('.number_format($entry['id'], '0', '', '.').')'; ?></option>
...
else if(isset($_GET['table']) && ($_GET['table'] == 'items'))
{
    $row = $_GET['row'];
    if($_GET['cell'] == 0)
    {
...
else if(isset($_GET['table']) && $_GET['table'] == 'eventfights')
{
  ...
  if($_GET['cell'] == 0)
  {
    ...
    <div style="height:60px; width:40px; display: inline-block;">
      <img width="40px" height="40px" src="img/npc/<?php echo $entry['image']; ?>.png" class="attack">
