...
class Tournament
{
	...
	{
		$where = 'pendingtournament="' . $this->GetID() . '" AND tournament="0" AND arank > -1 AND fight = "0"';
		...
		$this->database->Update('tournament="' . $this->GetID() . '"', 'accounts', $where, 9999999);
		$this->database->Update('pendingtournament="0"', 'accounts', 'pendingtournament="' . $this->GetID() . '" AND tournament != pendingtournament', 9999999);
	}
	...
	public function UpdateBrackets()
	{
		...
		if ($deadCount == floor($totalCount / 2))
		{
			...
			if ($deadCount == 1 && $totalCount == 2)
			{
				$this->SetEnd(true);
				$update = $update . ',`end`="1"';
				$this->database->Update('tournament="0", pendingtournament="0"', 'accounts', '`tournament`="' . $this->GetID() . '"', 9999999);
				...
				$this->database->Update($pUpdate, 'accounts', '`id`="' . $player->GetID() . '"', 1);
			}
		}

		$this->database->Update($update, 'tournaments', '`id`="' . $this->GetID() . '"', 1);
	}

	public function GetFighter($id) : ?TournamentFighter
	{
	...
	{
		return strtotime("now") > strtotime($this->GetStartTime());
	}

	public function AddFighter($id, $npc, $name, $image)
	{
		$this->database->Insert(
			'`fighterid`, `name`, `image`, `npc`, `tournament`',
			'"' . $id . '","' . $name . '","' . $image . '","' . $npc . '","' . $this->GetID() . '"',
			'tournamentfighter'
		);
	...
	public function RemoveFighter($id, $npc)
	{
		foreach ($this->fighters as &$fighter)
		{
			if ($fighter->GetFighterID() == $id && $fighter->IsNPC() == $npc)
			{
				...
				$this->database->Delete('tournamentfighter', '`id`="' . $id . '"', 1);
			}
		}
	}
	...
	{
		$CheckFighter = $this->database->Select('*', 'tournamentfighter', 'tournament="'.$this->GetID().'"');
        $count = $CheckFighter->num_rows;
        return $count;
	}
