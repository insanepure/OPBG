...
function do_bbcode_ref($action, $attributes, $content, $params, $node_object)
{
  ...
  {
    if (
      substr($url, 0, 5) == 'data:' || substr($url, 0, 5) == 'file:'
...
function do_bbcode_anker($action, $attributes, $content, $params, $node_object)
{
  ...
  return '<a title="one piece,browsergame" id="' . $url . '">' . $text . '</a>';
}
...
function do_bbcode_img($action, $attributes, $content, $params, $node_object)
{
  ...
  {
    if (
      substr($content, 0, 5) == 'data:' || substr($content, 0, 5) == 'file:'
  ...
  $returnString = '<img title="one piece,browsergame" alt="one piece,browsergame" src="img.php?url=' . htmlspecialchars($content) . '"';
  $urlLength = strlen($serverUrl);
  if (substr($content, 0, $urlLength) == $serverUrl)
  {
    $returnString = '<img title="one piece,browsergame" alt="one piece,browsergame" src="' . htmlspecialchars($content) . '"';
  }
...
$bbcode->addCode(
  ...
  'simple_replace',
  null,
  array('start_tag' => '<strike>', 'end_tag' => '</strike>'),
  'inline',
  array('listitem', 'block', 'inline', 'link'),
  array()
);
...
$bbcode->addCode(
  ...
  'simple_replace',
  null,
  array('start_tag' => '<center>', 'end_tag' => '</center>'),
  'inline',
  array('listitem', 'block', 'inline', 'link'),
  array()
);
